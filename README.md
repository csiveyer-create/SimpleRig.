...
## Advanced Rigging v3.1
This build adds:
- A lighter 2D drawing surface for contrast.
- Fixed-length rope routing with draggable route nodes.
- Physics Sandbox mode that preserves the total rope length.
- Connection snapping between advanced objects.
- Auto Rope Routing by clicking route points and double-clicking the end.
- Build This text command for generating starter pulley systems.
- Telehandler library with representative JCB, Manitou and Merlo models.
- Performer movement envelopes.
- Camera objects with field-of-view cones.
- An augmented-reality camera overlay for HTTPS mobile/tablet deployments.

Physics is an interactive planning approximation, not a certified dynamic or structural solver.
