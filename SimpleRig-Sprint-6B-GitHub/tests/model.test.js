...
import { snapValue, snapPoint, angleDegrees, scaleFromDrag } from "../src/geometry.js";
assert.equal(snapValue(1.12,.25),1);
assert.equal(snapValue(1.14,.25),1.25);
assert.deepEqual(snapPoint({x:.26,y:.62,z:-.12},.25),{x:.25,y:.5,z:0});
assert.equal(Math.round(angleDegrees({x:0,y:0},{x:0,y:1})),90);
assert.equal(scaleFromDrag({x:0,y:0},{x:10,y:0},{x:20,y:0},1),2);
console.log("Editing geometry tests passed.");
