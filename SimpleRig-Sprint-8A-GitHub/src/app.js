...
import{snapPoint,angleDegrees,scaleFromDrag}from"./geometry.js";
const $=id=>document.getElementById(id);
const els={canvas:$("mainCanvas"),projectName:$("projectName"),gagName:$("gagName"),undoBtn:$("undoBtn"),redoBtn:$("redoBtn"),saveBtn:$("saveBtn"),openInput:$("openInput"),sceneList:$("sceneList"),emptySelection:$("emptySelection"),selectionEditor:$("selectionEditor"),objectLabel:$("objectLabel"),objectX:$("objectX"),objectY:$("objectY"),objectZ:$("objectZ"),objectRotation:$("objectRotation"),objectScale:$("objectScale"),duplicateBtn:$("duplicateBtn"),deleteBtn:$("deleteBtn"),status:$("status"),viewBadge:$("viewBadge"),clearSketchBtn:$("clearSketchBtn"),recognitionToggle:$("recognitionToggle"),snapToggle:$("snapToggle"),recognitionCard:$("recognitionCard"),recognitionLabel:$("recognitionLabel"),recognitionConfidence:$("recognitionConfidence"),recognitionDetail:$("recognitionDetail"),convertRecognitionBtn:$("convertRecognitionBtn"),keepRecognitionBtn:$("keepRecognitionBtn")};
...
const renderer=new Renderer(els.canvas,{
  select:id=>select(id),
  previewMove:(id,start,d)=>{
    if(!preview)preview=cloneProject(project);
    const o=project.objects.find(x=>x.id===id);if(!o)return;
    let next={x:start.position.x+d.x,y:start.position.y+d.y,z:start.position.z+d.z};
    if(els.snapToggle.checked)next=snapPoint(next,.25);
    o.position=next;
    if(o.type==="rigLine"&&start.end){
      const actual={x:next.x-start.position.x,y:next.y-start.position.y,z:next.z-start.position.z};
      o.end={x:start.end.x+actual.x,y:start.end.y+actual.y,z:start.end.z+actual.z};
    }
    refresh(false)
  },
  previewHandle:(id,start,handle,startWorld,currentWorld,view)=>{
    if(!preview)preview=cloneProject(project);
    const o=project.objects.find(x=>x.id===id);if(!o)return;
    const current=els.snapToggle.checked?snapPoint(currentWorld,.25):currentWorld;
    if(handle.type==="line-start")o.position={...current};
    else if(handle.type==="line-end")o.end={...current};
    else{
      const centre=renderer.project2D(start.position),startScreen=renderer.project2D(startWorld),currentScreen=renderer.project2D(currentWorld);
      if(handle.type==="scale")o.scale=scaleFromDrag(centre,startScreen,currentScreen,start.scale||1);
      else if(handle.type==="rotate")o.rotation=angleDegrees(centre,currentScreen)+90;
    }
    refresh(false)
  },
...
els.snapToggle.addEventListener("change",()=>status(els.snapToggle.checked?"Snapping on":"Snapping off"));
